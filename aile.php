...
<head>
    ...
    <title>Aile Sorgu | Logsuzlar Stabil System</title>
    ...
    <style>
        ...
        * {
        ...
        table {
            ...
            min-width: 1000px; /* Mevcut min-width değeri korundu */
        }
...
<div class="container">
    <h2>Aile Bilgisi Sorgu</h2>
    <form method="GET" id="queryForm">
        <input type="text" name="tc" placeholder="TC Kimlik Numarası" value="<?php echo $tc; ?>" required pattern="\d{11}" title="11 haneli TC girin">
    ...
    <div id="resultsContainer">
        ...
        if ($sorguYapildi) {
            ...
            if ($json) {
                ...
                // "success" anahtarının varlığını ve değerini kontrol et, ayrıca "data"nın dizi olup olmadığını ve boş olmadığını kontrol et
                if ($veri && isset($veri["success"]) && $veri["success"] && isset($veri["data"]) && is_array($veri["data"]) && count($veri["data"]) > 0) {
                    echo "<div class='table-wrapper'><table><thead><tr>";
                    // Başlıkları dinamik olarak oluştur
                    // Dikkat: API'den gelen ilk veri setinin anahtarlarını kullanıyoruz.
                    foreach (array_keys($veri["data"][0]) as $baslik) {
                        echo "<th>" . htmlspecialchars($baslik) . "</th>";
                    }
                    echo "</tr></thead><tbody>"; // tbody açılışı

                    foreach ($veri["data"] as $satir) {
                        echo "<tr>";
                        foreach ($satir as $deger) {
                            echo "<td>" . htmlspecialchars($deger ?? '') . "</td>"; // Null değerler için hata vermemesi için ?? '' eklendi
                        }
                        echo "</tr>";
                    }
                    echo "</tbody></table></div>"; // tbody ve table kapanışı
                } else {
